...
def update_SG(vnc, project, sg_old, sg_new, virtual_network):
	...
	try:
		if sg_old != sg_new:
			...
			vn_list = vnc.virtual_networks_list()['virtual-networks']

			for vn in vn_list:
				if vn['fq_name'][1] == project and vn['fq_name'][2] == virtual_network:
					vn_obj = vnc.virtual_network_read(vn['fq_name'])
					vmi_list = vn_obj.get_virtual_machine_interface_back_refs()
					...
					for vmi in vmi_list:
						...
						if sg_ref_old == sg_old:
							...
							print 'INFO: Security group on VMI "{}" updated from "{}" to "{}" in {} seconds'.format(vmi['to'][2], sg_ref_old, sg_ref_new, elapsed)

						elif sg_ref_old == sg_new:
							print 'INFO: Security group "{}" on VMI "{}" already exists'.format(sg_ref_old, vmi['to'][2])				
